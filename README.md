...
│   └── index.ts                # Express app 单入口（catch-all）
├── server/                     # 后端业务逻辑
│   ├── app.ts                  # Express 应用（路由聚合 + 中间件）
│   ├── server.ts               # 本地开发服务器入口
│   ├── routes/                 # 路由
│   │   ├── trials.ts           # 试炼管理
...
│   ├── lib/                    # 共享库
│   │   ├── supabase.ts         # Supabase 客户端（懒加载）
│   │   ├── glm.ts              # GLM 调用（含 callGLMStream 流式）
│   │   ├── logger.ts           # 结构化日志
│   │   ├── response.ts         # 统一响应格式
│   │   └── anticheat.ts        # 反作弊检测
│   └── middleware/
...
- `api/index.ts` — 唯一 Serverless 入口（Express catch-all）
- `vercel.json` — 路由配置（API 请求 → `/api/index`，其余 → SPA）
- 后端代码位于 `server/` 目录，通过 `includeFiles` 打包进函数
...
Vercel 默认域名在中国大陆访问受限，可通过 Nginx 反向代理解决：

1. 准备国内服务器（需 ICP 备案）+ 宝塔面板
2. 添加站点并申请 SSL 证书
3. 配置 Nginx 反向代理到 Vercel 域名
4. 关键配置：`proxy_set_header Host <vercel-domain>; proxy_buffering off;`
...
- **全局 IP 限流** — 每 IP 15 分钟 200 次请求，对话/评估/操作分别独立限流
